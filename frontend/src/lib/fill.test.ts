...
const c = (over: Partial<CountrySnap> = {}): CountrySnap => ({ n: 5, lens: [5, 0, 0], dom: 0, att: 0.5, z: 2, tone: null, top: [], ...over });
const hex = (s: string) => rgb(s).formatHex();

test('a snapshot from the four-lens worker (cached, or one tick old): dom 3 is navy and unlit, filter 3 is navy, other filters still colour', () => {
  const old = c({ n: 3, lens: [2, 0, 0, 3], dom: 3, att: 1 });
  assert.equal(fillFor(old, null), BASE_NAVY);
  assert.equal(fillAlphaFor(old, null), UNLIT_ALPHA);
  assert.equal(fillFor(old, 3), BASE_NAVY);
  assert.equal(hex(fillFor(old, 0)), LENSES[0].color.toLowerCase());
});

test('no stories → base navy; undefined → base navy', () => {
  assert.equal(fillFor(undefined, null), BASE_NAVY);
  assert.equal(fillFor(c({ n: 0, lens: [0, 0, 0], dom: -1 }), null), BASE_NAVY);
});
...
test('lens filter hides countries with nothing under that lens and recolours the rest', () => {
  const country = c({ lens: [3, 2, 0], dom: 0, att: 1 });
...
test('cap alpha: unlit is the navy tint, lensed rises with attention, filtered-out drops to unlit', () => {
  ...
  assert.equal(fillAlphaFor(c({ n: 0, lens: [0, 0, 0], dom: -1 }), null), UNLIT_ALPHA);
  const low = fillAlphaFor(c({ att: 0 }), null), high = fillAlphaFor(c({ att: 1 }), null);
  assert.ok(low > UNLIT_ALPHA && high > low && high <= 1, `${low} < ${high}`);
  assert.equal(fillAlphaFor(c({ lens: [3, 2, 0], dom: 0 }), 2), UNLIT_ALPHA);
});
