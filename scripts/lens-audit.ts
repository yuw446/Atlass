...
// section-rejected headlines, then which themes carried an article on their own. Method and past results:
// docs/precision-check.md.

import { readFileSync } from 'node:fs';
import { GDELT, COL, unzipSingle, parseRows, articleFrom, pageTitle, zeroTotals } from '../worker/tick.ts';
import { LENSES, parseThemes, lensOf } from '../shared/lenses.ts';
...
const lensed: string[] = [], sectioned: string[] = [];
...
console.log(`${rows.length} rows, ${lensed.length} lensed, ${sectioned.length} section-rejected\n`);
